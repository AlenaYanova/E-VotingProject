...
class EncryptedAnswer(object):
  ...
  def verify(self, pk, min=0, max=1):
    ...
    # verify the sum
    # print(homomorphic_sum.verify_disjunctive_encryption_proof(sum_possible_plaintexts, self.overall_proof, algs.EG_disjunctive_challenge_generator))
...
class Election(object):
  ...
  def fromJSONDict(cls, d):
    ...
    el.name = d['name']
    if 'voters_hash' in d: el.voters_hash = d['voters_hash']
    # if 'openreg' in d: el.openreg = d['openreg']
    el.voting_starts_at = d['start_datetime']
    el.voting_ends_at = d['end_datetime']
    el.questions = d['questions']
    if 'ballot_type' in d: el.ballot_type = d['ballot_type']
    if 'tally_type' in d: el.tally_type = d['tally_type']
  ...
  def add_vote(self, encrypted_vote):
    ...
    for question_num in range(len(self.questions)):
      ...
      for answer_num in range(len(answers)):
        ...
        encrypted_vote.encrypted_answers[question_num].choices[answer_num].pk = self.pk
