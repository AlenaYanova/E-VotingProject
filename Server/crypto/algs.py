...
class Utils:
    ...
    RAND.get_bits = random.getrandbits
    ...
    def random_mpz(cls, n_bits):
        ...
        return number.getRandomNumber(n_bits, cls.RAND.get_bits)
    
    @classmethod
    def random_mpz_lt(cls, max):
        # return randrange(0, max)
        n_bits = int(math.floor(math.log(max, 2)))
        return (number.getRandomNumber(n_bits, cls.RAND.get_bits) % max)
    
    @classmethod
    def random_prime(cls, n_bits):
        return number.getPrime(n_bits, cls.RAND.get_bits)
...
class EGSecretKey:
    ...
    def prove_decryption(self, ciphertext):
        ...
        c = int(sha((str(a) + "," + str(b)).encode('utf-8')).hexdigest(), 16)
...
def EG_disjunctive_challenge_generator(commitments):
  ...
  return int(sha(string_to_hash.encode('utf-8')).hexdigest(), 16)
