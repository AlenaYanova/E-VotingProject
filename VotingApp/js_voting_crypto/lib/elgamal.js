...
var rng = require("./rng");
var BigInt = require("./bigint").BigInt;
...
ElGamal.Params = function(p, q, g) {
  this.p = p;
  this.q = q;
  this.g = g;
};

ElGamal.Params.prototype = {
  generate: function() {
    // get the value x
    var x = random.randomInteger(this.q);
...
ElGamal.Params.fromJSONObject = function(d) {
  ...
  params.p = new BigInt(d.p);
  params.q = new BigInt(d.q);
  params.g = new BigInt(d.g);
  return params;
};

ElGamal.PublicKey = function(p,q,g,y) {
  this.p = p;
  this.q = q;
  this.g = g;
  this.y = y;
...
ElGamal.PublicKey.prototype = {
  ...
  verifyKnowledgeOfSecretKey: function(proof, challenge_generator) {
    ...
    var check = this.g.modPow(proof.response, this.p).equals(this.y.modPow(proof.challenge, this.p).multiply(proof.commitment).mod(this.p).mod(this.p));
  ...
  multiply: function(other) {
    ...
    var new_pk = new ElGamal.PublicKey(this.p, this.q, this.g, this.y.multiply(other.y).mod(this.p).mod(this.p));
    return new_pk;
  },
...
ElGamal.SecretKey = function(x, pk) {
  this.x = x;
  this.pk = pk;
};

ElGamal.SecretKey.prototype = {
  toJSONObject: function() {
  ...
  proveKnowledge: function(challenge_generator) {
    // generate random w
    var w = random.randomInteger(this.pk.q);
    ...
    var response = w.add(this.x.multiply(challenge)).mod(this.pk.q).mod(this.pk.q);
...
ElGamal.Ciphertext = function(alpha, beta, pk) {
  this.alpha = alpha;
  this.beta = beta;
  this.pk = pk;
};
  
ElGamal.Ciphertext.prototype = {
  toString: function() {
  ...
  multiply: function(other) {
    ...
    return new ElGamal.Ciphertext(this.alpha.multiply(other.alpha).mod(this.pk.p).mod(this.pk.p),
                                      this.beta.multiply(other.beta).mod(this.pk.p).mod(this.pk.p),
                                      this.pk);
  },
  ...
  decrypt: function(list_of_dec_factors) {
    ...
    _(list_of_dec_factors).each(function(dec_factor) {
      running_decryption = dec_factor.modInverse(self.pk.p).multiply(running_decryption).mod(self.pk.p).mod(self.pk.p);
    });
  ...
  simulateProof: function(plaintext, challenge) {
    ...
    var beta_over_plaintext = this.beta.multiply(plaintext.m.modInverse(this.pk.p)).mod(this.pk.p).mod(this.pk.p);
  ...
  verifyProof: function(plaintext, proof, challenge_generator) {
    ...
    // g, y, alpha, beta/m
    var beta_over_m = this.beta.multiply(plaintext.m.modInverse(this.pk.p)).mod(this.pk.p).mod(this.pk.p);
  ...
  generateDisjunctiveProof: function(list_of_plaintexts, real_index, randomness, challenge_generator) {
    ...
    var real_proof = this.generateProof(list_of_plaintexts[real_index], randomness, function(commitment) {
      ...
      return real_challenge.mod(self.pk.q).mod(self.pk.q);
    });
  ...
  verifyDisjunctiveProof: function(list_of_plaintexts, disj_proof, challenge_generator) {
    ...
    _(proofs).each(function(proof) {sum = sum.add(proof.challenge).mod(self.pk.q).mod(self.pk.q);});
...
ElGamal.Plaintext = function(m, pk, encode_m) {
  if (m == null) {
    alert('oy null m');
    return;
  }
  ...
  if (encode_m) {
    ...
    if (test.equals(BigInt.ONE)) {
      ...
      this.m = y.negate().mod(pk.p).mod(pk.p);
    }
  } else {
    this.m = m;
  }    
...
ElGamal.Plaintext.prototype = {
  getPlaintext: function() {
    ...
    if (this.m.compareTo(this.pk.q) < 0) {
  	  ...
  	  y = this.m.negate().mod(this.pk.p).mod(this.pk.p);
    }
...
ElGamal.Proof = function(A, B, challenge, response) {
  this.commitment = {};
  this.commitment.A = A;
  this.commitment.B = B;
...
ElGamal.Proof.prototype= {
  ...
  verify: function(little_g, little_h, big_g, big_h, p, q, challenge_generator) {
    ...
    var first_check = little_g.modPow(this.response, p).equals(big_g.modPow(this.challenge, p).multiply(this.commitment.A).mod(p).mod(p));

    // check that little_h^response = B * big_h^challenge
    var second_check = little_h.modPow(this.response, p).equals(big_h.modPow(this.challenge, p).multiply(this.commitment.B).mod(p).mod(p));
...
ElGamal.Proof.generate = function(little_g, little_h, x, p, q, challenge_generator) {
  ...
  // Compute response = w + x * challenge
  proof.response = w.add(x.multiply(proof.challenge)).mod(q).mod(q);
...
ElGamal.encrypt = function(pk, plaintext, r) {
  ...
  if (!r)
    r = random.randomInteger(pk.q);
  
  var alpha = pk.g.modPow(r, pk.p);
  var beta = (pk.y.modPow(r, pk.p)).multiply(plaintext.m).mod(pk.p).mod(pk.p);
...
// DLog Proof
//
ElGamal.DLogProof = function(commitment, challenge, response) {
  this.commitment = commitment;
  this.challenge = challenge;
  this.response = response;
};

ElGamal.DLogProof.prototype = {
  toJSONObject: function() {
