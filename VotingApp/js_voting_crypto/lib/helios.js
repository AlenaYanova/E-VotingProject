...
var ElGamal = require("./elgamal").ElGamal;
...
// election
HELIOS.Election = function() {
};

HELIOS.Election.prototype = { 
  ...
  get_hash: function() {
    ...
    // otherwise  
    return sha2.hex_sha256(this.toJSON());
  },
...
HELIOS.Election.fromJSONString = function(raw_json) {
  ...
  election.original_string = raw_json;
  election.election_hash = sha2.hex_sha256(election.original_string);
...
UTILS.generate_plaintexts = function(pk, min, max) {
  ...
  for (var i=0; i<=max; i++) {
    ...
    last_plaintext = last_plaintext.multiply(pk.g).mod(pk.p).mod(pk.p);
  }
...
HELIOS.makeDisjunctiveChallengeGenerator = function(context) {
  return function(commitments) {
    ...
    return new BigInt(sha2.hex_sha256(JSON.stringify(context)), 16);
  };
}

HELIOS.EncryptedAnswer = function(question, answer, pk, progress) {    
...
HELIOS.EncryptedAnswer.prototype = { 
  doEncryption: function(question, answer, pk, randomness, progress) {
    ...
    for (var i=0; i<question.answers.length; i++) {
      ...
      if (generate_new_randomness) {
        randomness[i] = random.randomInteger(pk.q);        
      }
	...
	individual_proofs[i] = choices[i].generateDisjunctiveProof(zero_one_plaintexts, plaintext_index, randomness[i], HELIOS.makeDisjunctiveChallengeGenerator({publicKey: pk.toJSONObject()}));
      }
    ...
    if (generate_new_randomness && question.max != null) {
      ...
      for (var i=1; i<question.answers.length; i++) {
        ...
        rand_sum = rand_sum.add(randomness[i]).mod(pk.q).mod(pk.q);
      }
...
HELIOS.EncryptedVote = function(election, answers, progress) {
  // empty constructor
  if (election == null)
    return;
  ...
  // freeze the ballot representation right now
  this.serialize();
};

HELIOS.EncryptedVote.prototype = {
  clearPlaintexts: function() {
    _(this.encrypted_answers).each(function(ea) {
      ea.clearPlaintexts();
    });
    this.has_plaintexts = false;
  },
  ...
  // we're not including plaintexts at this point anymore
  toJSONObject: function() {
  ...
  serialize: function() {
    if (!this.serialized_vote)
      this.serialized_vote = JSON.stringify(this.toJSONObject());
    
    return this.serialized_vote;
  },
  
  get_hash: function() {
     return sha2.hex_sha256(this.serialize());
  },
  
  get_audit_trail: function() {
    // this only works if we have randomness
    if (!this.has_plaintexts)
      throw "no plaintexts for audit trail";
    
    return {
      vote: this.serialize(),
      plaintext_answers: _(this.encrypted_answers)
        .map(function(ea) {ea.plaintext})
    };
  },
...
HELIOS.Tally = function(raw_tally, num_tallied) {
  this.tally = raw_tally;
  this.num_tallied = num_tallied;
};

HELIOS.Tally.prototype = {  
  toJSONObject: function() {
...
HELIOS.Trustee = function(uuid, public_key, public_key_hash, pok, decryption_factors, decryption_proofs) {
  this.uuid = uuid;
